...
public class NewBehaviourScript1 : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if(T > waitT){
			...
			else if(i > 90f && i < 270f && BackSweep == true){
				i = i + 3f;
			}
			else if(i >= 270f){
				BackSweep = false;
		...
		if(fireT > coolDownT){
			...
			if(i==angle){
				...
				Aim = new Vector2(Mathf.Cos(angle-(i-angle)), -Mathf.Abs(Mathf.Sin(angle - i)));
			}
			else{
				Aim = new Vector2(Mathf.Cos(-angle-(i-180f)), -Mathf.Abs(Mathf.Sin(-angle - i)));
			}
